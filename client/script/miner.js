...
class PeerListUI {
  ...
  constructor() {
    ...
    document.getElementById('connected-to').addEventListener('click', this.show.bind(this));
    document.getElementById('exit-blocks-overview').addEventListener('click', this.hide.bind(this));
  }

  show(desc) {
    const nodeType = 'Browser';
    this._text.innerHTML = `<b>Connected</b><br>NAN<br><small>Unknow</small>`;
...
class PeerEntry {
  ...
  set block(block) {
    // 	this._block = block;
    this._blockNumberEl.textContent = '#1';
    // 	this._updateTimeString();
...
class MapUI {
  constructor($) {
    ...
    this._peerListUI = new PeerListUI()
  ...
  _onPeerJoined(peer) {
    ...
    if (host && !this._connectedPeers.contains(host)) {
      ...
      this._peerListUI._createPeerEntry(peer)
    }
  }
